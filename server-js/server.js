...
// ---------------------------------------------------------------------------
async function s2sServer(pubSocket,db) {
  const rep = new Reply();
  ...
  for await (const [rawMsg] of rep) {
    try {
      ...
      }else if (mtype === "REPLICATION") {
        const operation = msg.operation;
        const data = msg.data;
        applyReplication(db, operation, data);
        log(`REPLICA recebida | op=${operation}`);
       await rep.send(makeResponse("REPLICATION_OK", {}));
...
async function handleLogin(db, payload, ts, recvLc) {
  clockUpdate(recvLc);
  ...
  (username, timestamp, logical_clock)
  VALUES (?,?,?)
`).run(username, ts, lc);
  log(`LOGIN_OK: '${username}' | lc=${lc}`);
  replicateToPeers("LOGIN", {
  username,
  timestamp: ts,
  logical_clock: lc,
});
...
async function handleCreateChannel(db, payload, ts, recvLc) {
  clockUpdate(recvLc);
  ...
  replicateToPeers("CREATE_CHANNEL", {
  channel,
  created_by: username,
  timestamp: ts,
  logical_clock: lc,
...
async function handlePublish(db, pubSocket, payload, ts, recvLc) {
  ...
  const lc  = clockTick();
  const now = physicalNowMs();
  const messageId = `${SERVER_ID}-${lc}-${now}`;
  ...
  (id, channel, username, message, timestamp, logical_clock)
  VALUES (?,?,?,?,?,?)
`).run(
  messageId,
  channel,
  ...
  await replicateToPeers("PUBLISH", {
  id: messageId,
  channel,
  username,
  message,
...
async function replicateToPeers(operation, data) {

  for (const [peerName, peerAddr] of Object.entries(S2S_PEERS)) {

    if (peerName === SERVER_ID) continue;
    ...
    try {
      ...
      if (resp.type !== "REPLICATION_OK") {
        log(`ACK inválido de ${peerName}`);
      }

      log(`Replica enviada para ${peerName}`);

    } catch (e) {

      log(`Erro replicando para ${peerName}: ${e.message}`);
...
function applyReplication(db, operation, data) {
...
`).run(
  ...
  data.logical_clock
    ...
    db.prepare(
      "INSERT OR IGNORE INTO channels (name, created_by, timestamp, logical_clock) VALUES (?,?,?,?)"
    ).run(
      data.channel,
      data.created_by,
      data.timestamp,
...
async function main() {
  ...
  for await (const [rawMsg] of repSocket) {
    try {
      ...
      let response;
      switch (type) {
        case "LOGIN":          response = await handleLogin(db, payload, ts, recvLc); break;
        case "LIST_CHANNELS":  response = handleListChannels(db, payload, recvLc); break;
        case "CREATE_CHANNEL": response = await handleCreateChannel(db, payload, ts, recvLc); break;
