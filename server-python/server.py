...
# ---------------------------------------------------------------------------
def s2s_server(context, pub_socket, conn):
    ...
    while True:
        try:
            ...
            elif mtype == "REPLICATION":
                operation = msg.get("operation")
                data = msg.get("data")

                apply_replication(conn, operation, data)

                log(f"REPLICA recebida | op={operation}")

                rep.send(pack("REPLICATION_OK", {}))
...
def handle_login(conn, payload, ts, recv_lc):
    ...
    with _db_lock:
        conn.execute(
            "INSERT OR IGNORE INTO logins (username, timestamp, logical_clock) VALUES (?,?,?)",
            (username, ts, lc)
        )
        conn.commit()
    ...
    replicate_to_peers("LOGIN", {
        "username": username,
        "timestamp": ts,
        "logical_clock": lc,
    })

    return pack("LOGIN_OK", {
        "username": username
    })
...
def handle_create_channel(conn, payload, ts, recv_lc):
    ...
    replicate_to_peers("CREATE_CHANNEL", {
    "channel": channel,
    "created_by": username,
    "timestamp": ts,
    "logical_clock": lc,
...
def handle_publish(conn, pub_socket, payload, ts, recv_lc):
    ...
    with _db_lock:
        conn.execute("INSERT OR IGNORE INTO publications (message_id, channel, username, message, timestamp, logical_clock)"
                     " VALUES (?,?,?,?,?,?)", (message_id, channel, username, message, now, lc))
        conn.commit()

    log(f"PUBLISH_OK: canal='{channel}' | user='{username}' | lc={lc}")
    replicate_to_peers("PUBLISH", {
...
def replicate_to_peers(operation: str, data: dict):
    for peer_name, peer_addr in S2S_PEERS.items():
        if peer_name == SERVER_ID:
            continue
        ...
        try:
            ...
            log(f"Replica enviada para {peer_name}")

        except Exception as e:
            log(f"Erro replicando para {peer_name}: {e}")

def apply_replication(conn, operation, data):

    with _db_lock:

        if operation == "LOGIN":

            _logged_in_users.add(data["username"])
            ...
            conn.execute(
                "INSERT OR IGNORE INTO publications (message_id, channel, username, message, timestamp, logical_clock) VALUES (?,?,?,?,?,?)",
                (
                    data["message_id"],
                    data["channel"],
                    data["username"],
